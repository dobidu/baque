...
- 249/249 tests passing (244 prior + 5 new)
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    **src/plugin_processor.h:**
    ...
    Also add to `plugin_processor.h` public section — test/state-load setter analogous to `set_pattern()` (05-01): <!-- audit-added -->
    ```cpp
    void set_feel_pattern(const FeelPattern& fp) noexcept;
    ```
    Verify `src/audio/feel_pattern.h` is already included (must be — `feel_pattern_` is a member).

    Add implementation to `plugin_processor.cpp`: <!-- audit-added -->
    ```cpp
            ...
            for (int p = 0; p < k_plock_param_count; ++p) {
                ...
                if (plock_pattern_.steps[s].active[p])
                    plock_pattern_.steps[s].values[p] =
                        juce::jlimit(-1.0e6f, 1.0e6f, static_cast<float>(plk.getProperty(k + "v", 0.0f))); // <!-- audit-added: guard NaN/Inf from corrupted presets -->
            }
        }
    }
    ```
...
<task type="auto">
  ...
  <action>
    **src/preset_manager.h:**
    ...
    class PresetManager {
      public:
        ...
        // Salva para um arquivo arbitrário — para testes e exportação customizada. <!-- audit-added -->
    ...
    **src/preset_manager.cpp:**
    ...
    // Core write logic — shared by save() and save_to_file(). <!-- audit-added -->
...
<task type="auto">
  ...
  <action>
    **tests/test_phase11_dod.cpp:**
    ...
    TEST_CASE("P11D2 - FeelPattern round-trips through getState/setState", "[p11_dod]") { // <!-- audit-added: non-vacuous — uses set_feel_pattern() -->
        juce::ScopedJuceInitialiser_GUI init;
        BaqueProcessor proc;
    ...
    TEST_CASE("P11D3 - PresetManager saves and loads a preset", "[p11_dod]") { // <!-- audit-added: save_to_file path — no user-dir pollution, no vacuous CI skip -->
        juce::ScopedJuceInitialiser_GUI init;
        BaqueProcessor proc;
    ...
    TEST_CASE("P11D5 - v4 state blob (no v5 subtrees) loads without crash", "[p11_dod]") { // <!-- audit-added: AC-4 coverage -->
        juce::ScopedJuceInitialiser_GUI init;
        BaqueProcessor proc;
  ...
  <verify>
    `cmake --build build -j$(nproc) 2>&1 | grep -E "^.*error:"` → 0 errors.
    `ctest --test-dir build -R baque_tests --output-on-failure` → 249/249 pass (244 prior + 5 new P11D1–P11D5).
  </verify>
  <done>AC-1, AC-2, AC-3, AC-4 all satisfied via test coverage; 249/249 tests pass</done>
</task>
...
- [ ] `cmake --build build -j$(nproc) 2>&1 | grep -E "^.*error:"` → 0 errors
- [ ] `ctest --test-dir build -R baque_tests --output-on-failure` → 249/249 pass (5 new P11D1–P11D5)
- [ ] `ctest --test-dir build -R p11_dod` → all 5 pass
- [ ] StepPattern round-trip verified (P11D1)
- [ ] FeelPattern non-default values round-trip verified (P11D2 — enabled=true, timing_ms=25.5f)
- [ ] SamplePad source_file_ stored and reloaded (P11D4)
- [ ] PresetManager::save_to_file() writes a *.bqpreset to tmp; load() restores it (P11D3)
- [ ] Backward-compat: P11D5 strips v5 subtrees, verifies no crash + feel defaults
</verification>
